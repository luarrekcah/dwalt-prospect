...
const addQuestion = () => {
  ...
  const questionLabel1 = document.createElement("label");
  questionLabel1.setAttribute("for", `question${questionCounter}`);
  questionLabel1.textContent = "Mensagem:";

  const questionInput = document.createElement("input");
  questionInput.setAttribute("type", "text");
  questionInput.classList.add("form-control");
  questionInput.setAttribute("id", `question${questionCounter}`);
  questionInput.setAttribute("name", `question${questionCounter}`);
  questionInput.setAttribute("required", "");
  ...
  const questionLabel2 = document.createElement("label");
  questionLabel2.setAttribute("for", `answer${questionCounter}`);
  questionLabel2.textContent = "Resposta:";
  ...
  answerInput.setAttribute("name", `answer${questionCounter}`);
  answerInput.setAttribute("required", "");
...
const getQuestionsAndAnswers = async () => {
  const questions = [];
  const questionElements = document.querySelectorAll(".card.mb-3");
  for (let i = 0; i < questionElements.length; i++) {
    const questionElement = questionElements[i];
    const questionTitle = questionElement.querySelector(
      "input[id^='question']"
    ).value;
    const answerInput = questionElement.querySelector("input[id^='answer']");
    const answer = answerInput.value;
    const files = [];
    const fileInputs = questionElement.querySelectorAll("input[type='file']");
